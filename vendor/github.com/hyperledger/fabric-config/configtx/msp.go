...
type MSP struct {
	...
	TLSRootCerts []*sm2.Certificate
	// List of TLS intermediate certificates trusted by this MSP;
	// They are returned by GetTLSIntermediateCerts.
	TLSIntermediateCerts []*sm2.Certificate
...
func (m *MSP) newMSPCRL(signingIdentity *SigningIdentity, certs ...*sm2.Certificate) (*pkix.CertificateList, error) {
...
// signing certificate.
func (m *MSP) validateCertificates(signingCert *sm2.Certificate, certs ...*sm2.Certificate) error {
...
func (m *MSP) isCACert(signingCert *sm2.Certificate) error {
...
func parseCRL(crls [][]byte) ([]*pkix.CertificateList, error) {
	...
	for _, crl := range crls {
		...
		certificateList, err := sm2.ParseCRL(pemBlock.Bytes)
		if err != nil {
...
func parsePrivateKeyFromBytes(priv []byte) (crypto.PrivateKey, error) {
	...
	privateKey, err := sm2.ParsePKCS8PrivateKey(pemBlock.Bytes)
	if err != nil {
...
func buildPemEncodedCertListFromX509(certList []*sm2.Certificate) [][]byte {
	certs := [][]byte{}
...
func pemEncodeX509Certificate(cert *sm2.Certificate) []byte {
	return pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: cert.Raw})
}

func pemEncodePKCS8PrivateKey(priv crypto.PrivateKey) ([]byte, error) {
	privBytes, err := sm2.MarshalPKCS8PrivateKey(priv)
	if err != nil {
...
func validateCACerts(caCerts []*sm2.Certificate) error {
	for _, caCert := range caCerts {
		if (caCert.KeyUsage & sm2.KeyUsageCertSign) == 0 {
