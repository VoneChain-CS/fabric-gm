...
func (o *OrdererOrg) CreateMSPCRL(signingIdentity *SigningIdentity, certs ...*sm2.Certificate) (*pkix.CertificateList, error) {
	msp, err := o.MSP()
	if err != nil {
...
func unmarshalEtcdRaftMetadata(mdBytes []byte) (orderer.EtcdRaft, error) {
	...
	for _, c := range etcdRaftMetadata.Consenters {
		...
		clientTLSCert, err := sm2.ParseCertificate(clientTLSCertBlock.Bytes)
		if err != nil {
		...
		serverTLSCert, err := sm2.ParseCertificate(serverTLSCertBlock.Bytes)
		if err != nil {
